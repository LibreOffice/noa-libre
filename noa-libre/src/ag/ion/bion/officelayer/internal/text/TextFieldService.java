    ...
    public ITextField[] getPlaceholderFields() throws TextException {
        try {
            ...
            for (int i = 0; i < pfs.length; i++) {
                if (pfs[i].Name.endsWith("Hidden") && pfs[i].Value.equals(Boolean.valueOf(true))) {
                    XRefreshable refresh = null;
                    refresh = (XRefreshable) UnoRuntime.queryInterface(XRefreshable.class, textDocument.getXTextDocument());
                    refresh.refresh();
                    break;
                }
            }
