...
public class ApplicationAssistant implements IApplicationAssistant {
  ...
  private static final String RELATIVE_BOOTSTRAP      = PROGRAM_FOLDER + File.separator
                                                          + "bootstrap"; //$NON-NLS-1$ //$NON-NLS-2$
  ...
  public ILazyApplicationInfo getLatestLocalApplication() {
    return getLatestLocalApplication(null);
  }
      ...
      for (int i = 0; i < lazyApplicationInfos.length; i++) {
        ...
        if (latestLazyApplicationInfo == null) {
          ...
          latestLazyApplicationInfo = appInfo;
            ...
            && appInfo.getUpdateVersion() > latestLazyApplicationInfo.getUpdateVersion()) {
          latestLazyApplicationInfo = appInfo;
        }
      }
    }
    ...
    if (System.getProperty("oo.application.path") != null) {
      ...
      try {
        ...
        if (OSHelper.IS_MAC) {
          ...
          File oooStandardFolder = new File(appsFolder, "OpenOffice.org.app"); //$NON-NLS-1$
          File folderToSearch = oooStandardFolder;
          if (!folderToSearch.exists()) {
            folderToSearch = appsFolder;
          }
          findPossibleOfficeHomes(officeProgressMonitor, folderToSearch, possibleOfficeHomes, 1, -1);
        }
        else { //linux, unix
          File file = new File("/opt"); //$NON-NLS-1$
          findPossibleOfficeHomes(officeProgressMonitor, file, possibleOfficeHomes, 1, 2);
  ...
  public ILazyApplicationInfo findLocalApplicationInfo(String home) {
    ...
    File file = null;
    if (OSHelper.IS_WINDOWS) {
      file = new File(home + File.separator
          + PROGRAM_FOLDER
          + File.separator
          + APPLICATION_EXECUTEABLE
          + ".exe"); //$NON-NLS-1$

    }
    else if (OSHelper.IS_MAC) {
      file = new File(home + File.separator
          + PRE_PROGRAM_FOLDER_MAC
          + File.separator
          + PROGRAM_FOLDER
          + File.separator
          + APPLICATION_EXECUTEABLE
    ...
    if (file.canRead())
      return new LazyApplicationInfo(home, findApplicationProperties(home));
        ...
        if (file != null) {
          ...
          if (homePathIdentified) {
            ...
            if (parent != null) {
              ...
              if (OSHelper.IS_MAC) { // there is one more folder "Contents", so one more level up
                parent = parent.getParentFile();
              }
              if (parent != null)
          ...
          if (file.isDirectory()) {
            ...
            if (progDirExists) {
              findPossibleOfficeHomes(officeProgressMonitor, progDir, list, 1, 1);
            }
            else if (currentLevel < maxLevel || maxLevel == -1) {
              findPossibleOfficeHomes(officeProgressMonitor, file, list, currentLevel + 1, maxLevel);
            }
          }
        }
      }
  ...
  private String[] getPossibleKeys() {
    ...
    for (; majorVersion <= 4; minorVersion = 0, majorVersion++) {
      ...
      arrayList.add("Software\\" + KEY_MAIN_PART + "\\" + KEY_MAIN_PART + "\\" + majorVersion + "." + minorVersion); //$NON-NLS-1$ //$NON-NLS-2$  //$NON-NLS-4$
      arrayList.add("Software\\" + KEY_MAIN_PART + "\\" + KEY_MAIN_PART + "\\" + majorVersion + "." + minorVersion + "." + updateVersion); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
      ...
      for (minorVersion = 1, updateVersion = 1; minorVersion <= 10; updateVersion = 1, minorVersion++) {
        ...
        arrayList.add("Applications\\" + KEY_MAIN_PART + " " + majorVersion + "." + minorVersion + "." + updateVersion + "\\shell\\edit\\command"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
        arrayList.add("Software\\" + KEY_MAIN_PART + "\\" + KEY_MAIN_PART + "\\" + majorVersion + "." + minorVersion); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        arrayList.add("Software\\" + KEY_MAIN_PART + "\\" + KEY_MAIN_PART + "\\" + majorVersion + "." + minorVersion + "." + updateVersion); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
